...
function Canvas() {
  ...
  const {nodes,edges}=useMemo(()=>{
    ...
    const columns=Math.min(Math.max(direct.length,1),4);
    ...
    add(growth.nodes[spaceId],(columns-1)*115+22,0,true);
    direct.forEach((n,i)=>{const x=(i%4)*230;const y=230+Math.floor(i/4)*(spaceId==='goal'?650:150);add(n,x,y);connect(spaceId,n);
      if(spaceId==='goal'&&n.type==='capability')all.filter(c=>c.parentId===n.id).filter(c=>!['attention','screen'].includes(c.id)).slice(0,4).forEach((c,j)=>{add(c,x+12,340+j*64);connect(n.id,c);});
    });
