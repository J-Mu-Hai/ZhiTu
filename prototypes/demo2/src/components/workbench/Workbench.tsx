...
export function Workbench() {
  ...
  return <div className="workbench open-workbench"><div className={`workbench-body ${chatOpen ? '' : 'chat-hidden'}`}><section className="workspace">
    ...
    <div className="view-content">{view === 'path' ? <PathView key={spaceId}/> : view === 'timeline' ? <TimelineView/> : <TaskView/>}</div></section><FloatingConversation open={chatOpen} onClose={() => setChatOpen(false)}/></div></div>;
}
